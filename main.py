...
def get_specific_questions(set_cnt, sources):
    ...
    if set_cnt == 0: # set1
        ...
        questions.append(random.choice(q2))
        ...
        questions.append(random.choice(q5q8))
        ...
        questions.append(random.choice(q11))
        ...
        questions.append(random.choice(q14))
...
def init_source():    
    ...
    # get questions info from excel 
    question_types = pd.ExcelFile(sources_path).sheet_names    
    num_of_question_types = len(question_types)
    question_set_numbers = random.sample(range(0, num_of_question_types), 5) # opic has 5 sets for a test 
    
    # set questions in order
    set_cnt = 0
    for n in question_set_numbers:
        sheet_name = question_types[n]
        print("question set " + str(set_cnt + 1) + ": " + sheet_name)
        question_sources = pd.read_excel(sources_path, sheet_name=sheet_name).values.tolist()
