...
from config import sources_path_role_play, sources_path_selected_topic, sources_path_random_topic, base_path
...
def next_question():
    ...
    question_uuid = str(questions[current_question_index][4])

    create_audio(question_uuid, question)
...
def get_specific_questions(set_cnt, sources):
    ...
    elif set_cnt == 3:  # set4 roll playing
...
def get_sheet_names(path, cnt):
    ...
    question_set_index_list = random.sample(range(0, num_of_question_types), cnt)
    l = []
    for i in question_set_index_list:
...
def init_source():
    ...
    question_sheet_names += get_sheet_names(base_path + sources_path_selected_topic, 2)
    question_sheet_names += get_sheet_names(base_path + sources_path_random_topic, 2)
    random.shuffle(question_sheet_names)
    role_play_question_sheet_names = get_sheet_names(base_path + sources_path_role_play, 1)
